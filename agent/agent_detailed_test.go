...
func (m *MockLLMForDetailed) Generate(ctx context.Context, prompt string, options ...contracts.GenerateOption) (string, error) {
...
func (m *MockLLMForDetailed) GenerateWithTools(ctx context.Context, prompt string, tools []contracts.Tool, options ...contracts.GenerateOption) (string, error) {
	return m.Generate(ctx, prompt, options...)
}

func (m *MockLLMForDetailed) GenerateDetailed(ctx context.Context, prompt string, options ...contracts.GenerateOption) (*contracts.LLMResponse, error) {
...
func (m *MockLLMForDetailed) GenerateWithToolsDetailed(ctx context.Context, prompt string, tools []contracts.Tool, options ...contracts.GenerateOption) (*contracts.LLMResponse, error) {
...
func TestUsageTrackerAggregation(t *testing.T) {
	...
	// Get results
	totalUsage, execSummary, primaryModel := tracker.Results()
...
func TestUsageTrackerDisabled(t *testing.T) {
	...
	tracker := execution.NewTracker(false)
...
func TestAgentRunDetailedMemoryIntegration(t *testing.T) {
	...
	assert.Equal(t, contracts.RoleUser, messages[0].Role)
	assert.Equal(t, "Test with memory", messages[0].Content)

	// Verify second message (assistant)
	assert.Equal(t, contracts.RoleAssistant, messages[1].Role)
...
func TestUsageTrackerContextManagement(t *testing.T) {
	...
	retrievedTracker := execution.TrackerFromContext(ctx)
	...
	nilTracker := execution.TrackerFromContext(emptyCtx)
