...
func (m *TestMockLLM) Generate(ctx context.Context, prompt string, options ...contracts.GenerateOption) (string, error) {
	return "mock response", nil
}

func (m *TestMockLLM) GenerateWithTools(ctx context.Context, prompt string, tools []contracts.Tool, options ...contracts.GenerateOption) (string, error) {
...
func (m *TestMockLLM) GenerateDetailed(ctx context.Context, prompt string, options ...contracts.GenerateOption) (*contracts.LLMResponse, error) {
...
func (m *TestMockLLM) GenerateWithToolsDetailed(ctx context.Context, prompt string, tools []contracts.Tool, options ...contracts.GenerateOption) (*contracts.LLMResponse, error) {
...
func TestContextManagement(t *testing.T) {
	...
	depth := agentcontext.GetRecursionDepth(ctx)
	if depth != 0 {
		t.Errorf("Expected initial depth 0, got %d", depth)
	}

	// Add sub-agent context
	ctx = agentcontext.WithSubAgentContext(ctx, "MainAgent", "SubAgent1")
	...
	// Test sub-agent name
	name := agentcontext.GetSubAgentName(ctx)
	if name != "SubAgent1" {
		t.Errorf("Expected sub-agent name SubAgent1, got %s", name)
	}

	// Test parent agent
	parent := agentcontext.GetParentAgent(ctx)
	if parent != "MainAgent" {
		t.Errorf("Expected parent agent MainAgent, got %s", parent)
	}

	// Test IsSubAgentCall
	if !agentcontext.IsSubAgentCall(ctx) {
		t.Error("Expected IsSubAgentCall to return true")
	}

	// Test recursion depth validation
	for i := 0; i < agentcontext.MaxRecursionDepth; i++ {
		ctx = agentcontext.WithSubAgentContext(ctx, fmt.Sprintf("Agent%d", i), fmt.Sprintf("Agent%d", i+1))
	}

	err := agentcontext.ValidateRecursionDepth(ctx)
	if err == nil {
