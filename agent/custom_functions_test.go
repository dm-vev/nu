...
type mockLLM struct {
	generateFunc func(ctx context.Context, prompt string, options ...contracts.GenerateOption) (string, error)
	name         string
}
...
func (m *mockTool) Parameters() map[string]contracts.ParameterSpec {
...
func TestWithCustomRunStreamFunction(t *testing.T) {
	...
	customStreamFunc := func(ctx context.Context, input string, agent *Agent) (<-chan contracts.AgentStreamEvent, error) {
		ch := make(chan contracts.AgentStreamEvent, 1)
		ch <- contracts.AgentStreamEvent{Type: contracts.AgentEventComplete}
		close(ch)
		return ch, nil
	}
...
func TestAgentRunWithCustomFunction(t *testing.T) {
	...
	ctx = conversation.WithConversationID(ctx, "test-conversation")
...
func TestAgentRunWithoutCustomFunction(t *testing.T) {
	...
	agent, err := NewAgent(
		WithLLM(llm),
		WithMemory(conversation.NewConversationBuffer()),
		WithName("TestAgent"),
...
func TestAgentRunStreamWithCustomFunction(t *testing.T) {
	...
	customStreamFunc := func(ctx context.Context, input string, agent *Agent) (<-chan contracts.AgentStreamEvent, error) {
		...
		go func() {
			...
			eventChan <- contracts.AgentStreamEvent{
				Type:         contracts.AgentEventThinking,
	...
	// Check event types
	if events[0].Type != contracts.AgentEventThinking {
		t.Errorf("Expected first event to be thinking, got %s", events[0].Type)
	}

	if events[1].Type != contracts.AgentEventContent {
		t.Errorf("Expected second event to be content, got %s", events[1].Type)
	}

	if events[len(events)-1].Type != contracts.AgentEventComplete {
...
func TestCustomFunctionWithAgentComponents(t *testing.T) {
	...
	var capturedLLM contracts.LLM
	var capturedMemory contracts.Memory
	var capturedTools []contracts.Tool
...
func TestBothCustomFunctions(t *testing.T) {
	...
	customStreamFunc := func(ctx context.Context, input string, agent *Agent) (<-chan contracts.AgentStreamEvent, error) {
		eventChan := make(chan contracts.AgentStreamEvent, 1)
		go func() {
			defer close(eventChan)
			eventChan <- contracts.AgentStreamEvent{
				Type:      contracts.AgentEventContent,
