...
// ExecutionPlanGeneratorService handles generation of execution plans.
type ExecutionPlanGeneratorService struct {
	llm             contracts.LLM
...
// NewExecutionPlanGenerator creates an execution plan generator.
func NewExecutionPlanGenerator(llm contracts.LLM, tools []contracts.Tool, systemPrompt string, requireApproval bool) *ExecutionPlanGeneratorService {
	return &ExecutionPlanGeneratorService{
		llm:             llm,
		tools:           tools,
...
func (g *ExecutionPlanGeneratorService) GenerateExecutionPlan(ctx context.Context, input string) (*ExecutionPlan, error) {
	...
	if g.requireApproval {
		plan.Status = ExecutionPlanStatusPendingApproval
	} else {
...
func (g *ExecutionPlanGeneratorService) ModifyExecutionPlan(ctx context.Context, plan *ExecutionPlan, modifications string) (*ExecutionPlan, error) {
...
`, FormatExecutionPlan(plan), modifications)
	...
	modifiedPlan.Status = ExecutionPlanStatusPendingApproval
