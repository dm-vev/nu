...
func NewAgentFromFile(ctx context.Context, configPath string, options ...Option) (*Agent, error) {
	configs, err := config.LoadAgentConfigsFromFile(configPath)
	if err != nil {
	...
	if len(configs) == 1 {
		for _, cfg := range configs {
			return NewAgentFromConfigObject(ctx, &cfg, nil, options...)
		}
	}
...
func NewAgentFromCreateConfig(ctx context.Context, create DeploymentConfigCreateOptions) (*Agent, error) {
	if create.AgentName == "" {
	...
	if create.ConfigPath != "" {
		return NewAgentFromFileWithName(ctx, create.ConfigPath, create.AgentName, create.CustomOptions...)
	}

	loadOptions := []config.DeploymentConfigLoadOption{func(opts *config.DeploymentConfigLoadOptions) {
		opts.PreferRemote = create.PreferRemote
		opts.AllowFallback = create.AllowFallback
		opts.EnableEnvOverrides = create.EnableEnvOverrides
		opts.Verbose = create.Verbose
		if create.CacheTimeout > 0 {
			opts.CacheTimeout = create.CacheTimeout
		}
	}}

	agentOptions := append([]Option(nil), create.CustomOptions...)
	if create.MaxIterations != nil {
		agentOptions = append(agentOptions, WithMaxIterations(*create.MaxIterations))
	}
	if create.RequirePlanApproval != nil {
		agentOptions = append(agentOptions, WithRequirePlanApproval(*create.RequirePlanApproval))
	}

	loaded, err := config.LoadDeploymentAgentConfig(ctx, create.AgentName, create.Environment, loadOptions...)
	if err != nil {
		return nil, err
	}
...
func WithConfigFile(filePath, agentName string) Option {
	return func(a *Agent) {
		...
		configs, err := config.LoadAgentConfigsFromFile(filePath)
		if err != nil {
