...
func Example_preview() {
	...
	config, err := config.PreviewDeploymentAgentConfig(ctx, "research-assistant", "production")
	if err != nil {
...
func Example_advancedOptions() {
	...
	loadOptions := []config.DeploymentConfigLoadOption{
		config.WithDeploymentConfigLocalFallback("./configs/research.yaml"), // Specific fallback file
		config.WithDeploymentConfigCache(10 * time.Minute),                  // Longer cache
		config.WithDeploymentConfigEnvOverrides(),
		config.WithDeploymentConfigVerbose(),
	}
