...
func NewRegistryManager(registryURL string) *RegistryManager {
	return &RegistryManager{
		...
		builder:        builder.NewBuilder(), logger: telemetry.NewLogger(),
	}
}

func (rm *RegistryManager) DiscoverAndInstallServer(ctx context.Context, serverID string) (*config.Config, error) {
...
func (rm *RegistryManager) BuildServerFromRegistry(ctx context.Context, serverID string) (contracts.MCPServer, error) {
	...
	server, err := rm.builder.Connect(ctx, *config)
	if err != nil {
...
func (rm *RegistryManager) registryServerToConfig(server *RegistryServer) (*config.Config, error) {
	config := &config.Config{Name: server.Name}
