...
func (b *Builder) parseServerURL(urlStr string) (contracts.MCPServer, *LazyMCPServerConfig, error) {
	...
	case "mcp":
		preset, err := preset.GetPreset(u.Host + u.Path)
		if err != nil {
			return nil, nil, err
		}
...
// Connect creates an MCP server from a resolved configuration.
func (b *Builder) Connect(ctx context.Context, config LazyMCPServerConfig) (contracts.MCPServer, error) {
	...
	switch config.Type {
	case "stdio":
		server, err = transport.NewStdioServerWithRetry(ctx, transport.StdioServerConfig{
			Command: config.Command, Args: config.Args, Env: config.Env, Logger: b.logger,
		}, retryConfig)
	case "http":
		server, err = transport.NewHTTPWithRetry(ctx, transport.HTTPConfig{
			BaseURL: config.URL, Token: config.Token, Logger: b.logger,
			ProtocolType: transport.ServerProtocolType(config.HttpTransportMode),
		}, retryConfig)
	default:
