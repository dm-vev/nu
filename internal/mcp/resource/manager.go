...
func NewResourceManager(servers []contracts.MCPServer) *ResourceManager {
...
func (rm *ResourceManager) ListAllResources(ctx context.Context) (map[string][]contracts.MCPResource, error) {
	result := make(map[string][]contracts.MCPResource)
...
func (rm *ResourceManager) GetResourceContent(ctx context.Context, uri string) (*contracts.MCPResourceContent, error) {
	for i, server := range rm.servers {
		serverName := fmt.Sprintf("server-%d", i)
...
func (rm *ResourceManager) WatchResources(ctx context.Context, uris []string) (<-chan ResourceUpdate, error) {
	...
	for _, uri := range uris {
		for i, server := range rm.servers {
			...
			go func(serverName, uri string, serverUpdates <-chan contracts.MCPResourceUpdate) {
...
type ResourceUpdate struct {
	Server string
	Update contracts.MCPResourceUpdate
}

// Helper methods

func (rm *ResourceManager) matchesPattern(resource contracts.MCPResource, pattern string) bool {
...
func (rm *ResourceManager) matchesMimeType(resource contracts.MCPResource, targetType string) bool {
...
func IsTextResource(resource contracts.MCPResource) bool {
	if resource.MimeType == "" {
...
func IsBinaryResource(resource contracts.MCPResource) bool {
	return !IsTextResource(resource)
}

// GetResourceExtension extracts file extension from resource URI
func GetResourceExtension(resource contracts.MCPResource) string {
