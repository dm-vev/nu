...
func TestMCPServerMetadataDiscovery(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			capabilities: &contracts.MCPServerCapabilities{
				Tools: &contracts.MCPToolCapabilities{
					ListChanged: true,
				},
				Resources: &contracts.MCPResourceCapabilities{
					Subscribe:   true,
					ListChanged: true,
				},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.serverInfo != nil {
				...
				mockServer.On("GetServerInfo").Return((*contracts.MCPServerInfo)(nil), nil)
			}
			...
			if tt.capabilities != nil {
				...
				mockServer.On("GetCapabilities").Return((*contracts.MCPServerCapabilities)(nil), nil)
			}
