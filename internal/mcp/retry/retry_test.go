...
func (m *mockMCPServer) ListTools(ctx context.Context) ([]contracts.MCPTool, error) {
	args := m.Called(ctx)
	if tools := args.Get(0); tools != nil {
		return tools.([]contracts.MCPTool), args.Error(1)
	}
	return nil, args.Error(1)
}

func (m *mockMCPServer) CallTool(ctx context.Context, name string, toolArgs interface{}) (*contracts.MCPToolResponse, error) {
	args := m.Called(ctx, name, toolArgs)
	if resp := args.Get(0); resp != nil {
		return resp.(*contracts.MCPToolResponse), args.Error(1)
	}
	return nil, args.Error(1)
}

func (m *mockMCPServer) ListResources(ctx context.Context) ([]contracts.MCPResource, error) {
	args := m.Called(ctx)
	if resources := args.Get(0); resources != nil {
		return resources.([]contracts.MCPResource), args.Error(1)
	}
	return nil, args.Error(1)
}

func (m *mockMCPServer) GetResource(ctx context.Context, uri string) (*contracts.MCPResourceContent, error) {
	args := m.Called(ctx, uri)
	if content := args.Get(0); content != nil {
		return content.(*contracts.MCPResourceContent), args.Error(1)
	}
	return nil, args.Error(1)
}

func (m *mockMCPServer) WatchResource(ctx context.Context, uri string) (<-chan contracts.MCPResourceUpdate, error) {
	args := m.Called(ctx, uri)
	if ch := args.Get(0); ch != nil {
		return ch.(<-chan contracts.MCPResourceUpdate), args.Error(1)
	}
	return nil, args.Error(1)
}

func (m *mockMCPServer) ListPrompts(ctx context.Context) ([]contracts.MCPPrompt, error) {
	args := m.Called(ctx)
...
func (m *mockMCPServer) GetPrompt(ctx context.Context, name string, variables map[string]interface{}) (*contracts.MCPPromptResult, error) {
...
func (m *mockMCPServer) CreateMessage(ctx context.Context, request *contracts.MCPSamplingRequest) (*contracts.MCPSamplingResponse, error) {
	args := m.Called(ctx, request)
	if resp := args.Get(0); resp != nil {
		return resp.(*contracts.MCPSamplingResponse), args.Error(1)
	}
	return nil, args.Error(1)
}

func (m *mockMCPServer) GetServerInfo() (*contracts.MCPServerInfo, error) {
	args := m.Called()
...
func (m *mockMCPServer) GetCapabilities() (*contracts.MCPServerCapabilities, error) {
	args := m.Called()
...
func TestRetryableServer_ListTools(t *testing.T) {
	ctx := context.Background()
	expectedTools := []contracts.MCPTool{
...
func TestRetryableServer_CallTool(t *testing.T) {
	...
	expectedResponse := &contracts.MCPToolResponse{Content: "result"}
...
func TestRetryableServer_WatchResource(t *testing.T) {
	ctx := context.Background()
	uri := "test://resource"
	expectedChannel := make(chan contracts.MCPResourceUpdate)

	t.Run("no retry for watch", func(t *testing.T) {
		mockServer := &mockMCPServer{}
		mockServer.On("WatchResource", ctx, uri).Return((<-chan contracts.MCPResourceUpdate)(expectedChannel), nil).Once()

		server := New(mockServer, nil)
		ch, err := server.WatchResource(ctx, uri)

		assert.NoError(t, err)
		assert.Equal(t, (<-chan contracts.MCPResourceUpdate)(expectedChannel), ch)
...
func TestRetryableServer_ComplexScenarios(t *testing.T) {
	...
	t.Run("mixed success and failure", func(t *testing.T) {
		...
		mockServer.On("ListTools", ctx).Return([]contracts.MCPTool{{Name: "tool1"}}, nil).Once()
