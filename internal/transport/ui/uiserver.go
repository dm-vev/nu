...
// NewServer creates an HTTP server with the embedded UI.
func NewServer(agent *agent.Agent, port int, config *Config) *Server {
	if config == nil {
		config = &Config{
			Enabled:     true,
			DefaultPath: "/",
	...
	if config.Features.Traces && config.Tracing != nil && config.Tracing.Enabled {
		...
		if agent.GetTracer() != nil {
			if uiCollector, ok := agent.GetTracer().(*TraceCollector); ok {
				...
				server.traceCollector = NewTraceCollector(config.Tracing, agent.GetTracer(), agent.GetLogger())
				log.Printf("[UI Server] Created new TraceCollector wrapping agent's tracer")
			}
		} else {
			// Agent has no tracer, create new TraceCollector
			server.traceCollector = NewTraceCollector(config.Tracing, nil, agent.GetLogger())
