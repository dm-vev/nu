...
func New(config *Config, wrappedTracer contracts.Tracer, logger telemetry.Logger) *Collector {
	if config == nil {
		config = &Config{
			Enabled:         true,
...
func (c *Collector) StartSpan(ctx context.Context, name string) (context.Context, contracts.Span) {
...
func (c *Collector) StartTraceSession(ctx context.Context, contextID string) (context.Context, contracts.Span) {
