...
func (m *MockTestTool) Parameters() map[string]contracts.ParameterSpec {
...
func TestHTTPServerWithUI_getToolNames(t *testing.T) {
	tests := []struct {
		name       string
		setupTools func() []contracts.Tool
		expected   []string
	}{
		{
			...
			setupTools: func() []contracts.Tool {
				...
				return []contracts.Tool{tool1, tool2}
			},
