...
func (m *Service) RunStream(ctx context.Context, input string) (<-chan contracts.AgentStreamEvent, error) {
	if !m.IsRunning() {
	...
	go func() {
		...
		for {
			...
			event := convertResponse(response)
...
func (m *Service) OnThinking(handler func(string)) *Service {
	m.handlersMu.Lock()
...
func (m *Service) OnContent(handler func(string)) *Service {
	m.handlersMu.Lock()
	defer m.handlersMu.Unlock()
...
func (m *Service) OnToolCall(handler func(*contracts.ToolCallEvent)) *Service {
	m.handlersMu.Lock()
	defer m.handlersMu.Unlock()
...
func (m *Service) OnToolResult(handler func(*contracts.ToolCallEvent)) *Service {
	m.handlersMu.Lock()
	defer m.handlersMu.Unlock()
...
func (m *Service) OnError(handler func(error)) *Service {
	m.handlersMu.Lock()
	defer m.handlersMu.Unlock()
...
func (m *Service) OnComplete(handler func()) *Service {
	m.handlersMu.Lock()
	defer m.handlersMu.Unlock()
...
func (m *Service) Stream(ctx context.Context, input string) error {
...
func (m *Service) executeHandlers(event contracts.AgentStreamEvent) {
	m.handlersMu.RLock()
...
// convertResponse converts gRPC stream response to AgentStreamEvent
func convertResponse(response *pb.RunStreamResponse) contracts.AgentStreamEvent {
