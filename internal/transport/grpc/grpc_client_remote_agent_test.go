...
func TestRemoteAgentClient_RunStream(t *testing.T) {
	...
	if events[0].Type != contracts.AgentEventContent {
	...
	if events[2].Type != contracts.AgentEventComplete {
		t.Errorf("Expected third event type to be complete, got %s", events[2].Type)
	}
	if events[3].Type != contracts.AgentEventComplete {
...
func TestRemoteAgentClient_RunStreamWithAuth_ErrorHandling(t *testing.T) {
	...
	if events[0].Type != contracts.AgentEventError {
...
func TestRemoteAgentClient_RunStreamWithAuth_ToolCallEvent(t *testing.T) {
	...
	if events[0].Type != contracts.AgentEventToolCall {
	...
	if events[1].Type != contracts.AgentEventToolResult {
...
func TestConvertPbToStreamEvent(t *testing.T) {
	...
	tests := []struct {
		...
		expected contracts.AgentStreamEvent
	}{
		{
			name: "content event",
			input: &pb.RunStreamResponse{
...
func TestRemoteAgentClient_EventHandlers(t *testing.T) {
	...
	var toolCallCalls []*contracts.ToolCallEvent
	...
	client.
		...
		OnToolCall(func(toolCall *contracts.ToolCallEvent) {
			mu.Lock()
