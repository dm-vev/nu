...
func TestWithToolName(t *testing.T) {
	agentCard := card.New("Agent A", "test agent", "http://localhost").Build()
	c, err := client.NewFromCard(context.Background(), agentCard)
	if err != nil {
	...
	// With override
	tool2 := New(c, WithToolName("custom_name"))
	...
	tool3 := New(c, WithToolName("agent_a_v2"))
