...
// New creates an A2A server that serves the given agent.
// The agentCard describes the agent's capabilities to A2A clients.
// It panics if agent or agentCard is nil.
func New(agent AgentAdapter, agentCard *a2a.AgentCard, opts ...Option) *Server {
	if agent == nil {
		panic("a2a: New requires a non-nil agent")
	}
	if agentCard == nil {
		panic("a2a: New requires a non-nil agentCard")
	}
	s := &Server{
		agent:             agent,
		card:              agentCard,
...
// during New and cached for subsequent calls.
