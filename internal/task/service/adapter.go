...
// Adapter defines a generic interface for adapting SDK task models
...
type AdapterService[AgentTask any, AgentCreateRequest any, AgentApprovalRequest any, AgentTaskUpdate any] struct {
	sdkService contracts.TaskService
	adapter    Adapter[AgentTask, AgentCreateRequest, AgentApprovalRequest, AgentTaskUpdate]
...
func (s *AdapterService[AgentTask, AgentCreateRequest, AgentApprovalRequest, AgentTaskUpdate]) ListTasks(
	...
	// Convert the generic service results to legacy tasks.
	var sdkTasks []*Task
	for _, obj := range sdkTaskObjList {
		if sdkTask, ok := obj.(*Task); ok {
