...
// Executor implements the contracts.Executor interface
type Executor struct {
	// Add fields as needed for configuration
	taskRegistry map[string]Func
	// Add more fields as needed
}

// Func is a function that executes a task
type Func func(ctx context.Context, params interface{}) (interface{}, error)
...
func (e *Executor) RegisterTask(name string, taskFunc Func) {
	e.taskRegistry[name] = taskFunc
}

// ExecuteStep executes a single step in a task plan
func (e *Executor) ExecuteStep(ctx context.Context, t *CoreTask, step *CoreStep) error {
	...
	// Update step status
	step.Status = CoreStatusExecuting
	...
	step.Status = CoreStatusCompleted
	return nil
}

// ExecuteTask executes all steps in a task's plan
func (e *Executor) ExecuteTask(ctx context.Context, t *CoreTask) error {
	if t == nil || len(t.Steps) == 0 {
		return fmt.Errorf("task has no steps")
	}

	// Update task status
	t.Status = CoreStatusExecuting

	// Execute steps in order
	for i := range t.Steps {
		step := t.Steps[i]
		if step.Status != CoreStatusPending {
			continue // Skip steps that are not pending
		}

		if err := e.ExecuteStep(ctx, t, step); err != nil {
			t.Status = CoreStatusFailed
			failTime := time.Now()
			t.FailedAt = &failTime
			return err
		}
...
func (e *Executor) ExecuteSync(ctx context.Context, taskName string, params interface{}, opts *contracts.TaskOptions) (*contracts.TaskResult, error) {
	taskFunc, exists := e.taskRegistry[taskName]
	if !exists {
		return nil, fmt.Errorf("task %s not registered", taskName)
	}

	// Create local Options to handle the nil case
	localOpts := &Options{}
	...
	if opts != nil && opts.Timeout != nil {
		...
		// Convert contracts.TaskOptions to our local Options
...
func (e *Executor) ExecuteAsync(ctx context.Context, taskName string, params interface{}, opts *contracts.TaskOptions) (<-chan *contracts.TaskResult, error) {
	taskFunc, exists := e.taskRegistry[taskName]
	if !exists {
		return nil, fmt.Errorf("task %s not registered", taskName)
	}

	resultChan := make(chan *contracts.TaskResult, 1)
...
func (e *Executor) executeWithRetry(ctx context.Context, taskFunc Func, params interface{}, opts *Options) (interface{}, error) {
