...
func TestNUF072EditSingleReplacement(t *testing.T) {
	...
	result, err := Run(context.Background(), dir, `{"path":"a.txt","replacements":[{"old":"old","new":"new"}]}`)
	if err != nil {
...
func TestNUF072EditRejectsAmbiguousOldText(t *testing.T) {
	...
	_, err := Run(context.Background(), dir, `{"path":"a.txt","replacements":[{"old":"x","new":"y"}]}`)
...
func TestNUF072EditPreservesCRLF(t *testing.T) {
	...
	if _, err := Run(context.Background(), dir, `{"path":"a.txt","replacements":[{"old":"old","new":"new"}]}`); err != nil {
		t.Fatalf("Run error = %v", err)
	}
...
func TestEditRejectsMissingOldText(t *testing.T) {
	...
	_, err := Run(context.Background(), dir, `{"path":"a.txt","replacements":[{"old":"missing","new":"x"}]}`)
