...
func (c *Client) GenerateStream(
	...
	go func() {
		...
		streamParams := openai.ChatCompletionNewParams{Model: openai.ChatModel(c.deployment), Messages: messages}
		...
		if params.ResponseFormat != nil {
			...
			streamParams.ResponseFormat = openai.ChatCompletionNewParamsResponseFormatUnion{OfJSONSchema: &shared.ResponseFormatJSONSchemaParam{Type: "json_schema", JSONSchema: jsonSchema}}
		}
		if azureOpenAIIsReasoningModel(c.Model) || (params.LLMConfig != nil && params.LLMConfig.EnableReasoning) {
		...
		if params.LLMConfig != nil {
			if params.LLMConfig.TopP > 0 && !azureOpenAIIsReasoningModel(c.Model) {
		...
		c.logger.Debug(ctx, "Creating Azure OpenAI streaming request", map[string]interface{}{
			"model": c.Model, "deployment": c.deployment, "temperature": params.LLMConfig.Temperature,
			"top_p": params.LLMConfig.TopP, "is_reasoning_model": azureOpenAIIsReasoningModel(c.Model),
		})
		...
		eventChan <- contracts.StreamEvent{
			...
			Metadata: map[string]interface{}{"model": c.Model, "deployment": c.deployment},
		}
		...
		for stream.Next() {
			...
			if chunk.Usage.PromptTokens > 0 || chunk.Usage.CompletionTokens > 0 || chunk.Usage.TotalTokens > 0 {
				eventChan <- contracts.StreamEvent{
					Type: contracts.StreamEventContentDelta, Timestamp: time.Now(),
					Metadata: map[string]interface{}{"usage": map[string]interface{}{
						"prompt_tokens": chunk.Usage.PromptTokens, "completion_tokens": chunk.Usage.CompletionTokens, "total_tokens": chunk.Usage.TotalTokens,
					}},
				}
			}
		}
		if err := stream.Err(); err != nil {
			c.logger.Error(ctx, "Azure OpenAI streaming error", map[string]interface{}{"error": err.Error(), "model": c.Model, "deployment": c.deployment})
			eventChan <- contracts.StreamEvent{Type: contracts.StreamEventError, Error: fmt.Errorf("azure openai streaming error: %w", err), Timestamp: time.Now()}
			return
		}
		eventChan <- contracts.StreamEvent{Type: contracts.StreamEventMessageStop, Timestamp: time.Now()}
		c.logger.Debug(ctx, "Successfully completed Azure OpenAI streaming request", map[string]interface{}{"model": c.Model, "deployment": c.deployment})
	}()
