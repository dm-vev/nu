...
This package provides a client for interacting with Azure OpenAI services, implementing the `contracts.LLM` interface. It's designed to work seamlessly with Azure's OpenAI deployments while maintaining compatibility with the existing Agent SDK patterns.
...
- Full compatibility with the `contracts.LLM` interface
...
eventChan, err := client.GenerateStream(
    ...
    contracts.WithStreamConfig(streamConfig),
)
...
- `WithLogger(telemetry.Logger)` - Sets a custom logger
- `WithRetry(...llm.RetryOption)` - Configures retry behavior
...
- `WithResponseFormat(contracts.ResponseFormat)` - Enables structured output
