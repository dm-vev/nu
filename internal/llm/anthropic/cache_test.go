...
func TestAnthropicCacheRequestBuilder_HasCacheOptions(t *testing.T) {
	tests := []struct {
		name     string
		...
		},
		{
			...
			config:   &contracts.CacheConfig{CacheSystemMessage: true},
			expected: true,
		},
		{
			name:     "cache tools",
			config:   &contracts.CacheConfig{CacheTools: true},
			expected: true,
		},
		{
			name:     "cache conversation",
			config:   &contracts.CacheConfig{CacheConversation: true},
			expected: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			builder := anthropicNewCacheRequestBuilder(tt.config)
			assert.Equal(t, tt.expected, builder.HasCacheOptions())
		})
	}
}

func TestAnthropicCacheRequestBuilder_BuildSystemContent(t *testing.T) {
	tests := []struct {
		name           string
		config         *contracts.CacheConfig
		system         string
		...
		},
		{
			...
			config:         &contracts.CacheConfig{CacheSystemMessage: false},
			system:         "You are helpful",
			expectArray:    false,
			expectCacheCtl: false,
		},
...
func TestAnthropicCacheRequestBuilder_BuildMessages(t *testing.T) {
	messages := []Message{
...
func TestAnthropicCacheRequestBuilder_BuildTools(t *testing.T) {
	tools := []Tool{
...
func TestAnthropicCacheRequestBuilder_BuildCacheableRequest(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                "cache all",
			config:              &contracts.CacheConfig{CacheSystemMessage: true, CacheTools: true, CacheConversation: true},
...
func TestAnthropicUsageParsingWithCacheMetrics(t *testing.T) {
	responseJSON := `{
		"id": "msg_123",
		"type": "message",
		"role": "assistant",
...
func TestAnthropicCacheOptions(t *testing.T) {
	t.Run("WithAnthropicCacheSystemMessage", func(t *testing.T) {
	...
	t.Run("WithAnthropicCacheTools", func(t *testing.T) {
		opts := &contracts.GenerateOptions{}
		WithCacheTools()(opts)
	...
	t.Run("WithAnthropicCacheConversation", func(t *testing.T) {
		opts := &contracts.GenerateOptions{}
	...
	t.Run("WithAnthropicCacheTTL", func(t *testing.T) {
		opts := &contracts.GenerateOptions{}
