...
func TestAnthropicCacheIntegration_SystemMessage(t *testing.T) {
...
func TestAnthropicCacheIntegration_LongConversation(t *testing.T) {
	apiKey := os.Getenv("ANTHROPIC_API_KEY")
	if apiKey == "" {
		t.Skip("ANTHROPIC_API_KEY not set")
	}
...
`, i+1, component, component)
	...
	longContext += anthropicGeneratePadding(2000)
	...
	resp1, err := client.GenerateDetailed(ctx, "Summarize the architecture in one sentence.",
		contracts.WithSystemMessage(longContext),
		WithCacheSystemMessage(),
	)
	require.NoError(t, err)
	require.NotNil(t, resp1.Usage)
...
func TestAnthropicCacheIntegration_Tools(t *testing.T) {
	apiKey := os.Getenv("ANTHROPIC_API_KEY")
	if apiKey == "" {
		t.Skip("ANTHROPIC_API_KEY not set")
	}
	...
	tools := []contracts.Tool{
		&anthropicCacheTestTool{
			name:        "calculator",
			description: "Performs calculations. " + anthropicGeneratePadding(500),
		...
		&anthropicCacheTestTool{
			name:        "weather",
			description: "Gets weather. " + anthropicGeneratePadding(500),
			params: map[string]contracts.ParameterSpec{
	...
	systemMessage := fmt.Sprintf("You are a helpful assistant. Session: %d. ", time.Now().UnixNano()) + anthropicGeneratePadding(1000)

	// First call - should create cache
	t.Log("Making first request with tools...")
	resp1, err := client.GenerateWithToolsDetailed(ctx, "What is 15 * 23?", tools,
		contracts.WithSystemMessage(systemMessage),
		WithCacheSystemMessage(),
		WithCacheTools(),
	)
	require.NoError(t, err)
...
func anthropicGeneratePadding(words int) string {
	padding := ""
	for i := 0; i < words; i++ {
...
// anthropicCacheTestTool implementation for testing
type anthropicCacheTestTool struct {
	name        string
	description string
	params      map[string]contracts.ParameterSpec
}

func (t *anthropicCacheTestTool) Name() string        { return t.name }
func (t *anthropicCacheTestTool) Description() string { return t.description }

func (t *anthropicCacheTestTool) Run(ctx context.Context, input string) (string, error) {
	return "42", nil
}

func (t *anthropicCacheTestTool) Execute(ctx context.Context, args string) (string, error) {
	return "42", nil
}

func (t *anthropicCacheTestTool) Parameters() map[string]contracts.ParameterSpec {
	return t.params
}
