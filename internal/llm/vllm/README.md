...
- `WithLogger(logger telemetry.Logger)`: Set a custom logger
...
- `WithResponseFormat(format contracts.ResponseFormat)`: Sets expected response format
...
personFormat := llm.NewStructuredResponseFormat(Person{})
...
```go
logger := telemetry.NewLogger()
logger.SetLevel(telemetry.DebugLevel)
