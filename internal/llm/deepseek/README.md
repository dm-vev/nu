...
func (t *WeatherTool) Parameters() map[string]contracts.ParameterSpec {
...
mem.AddMessage(ctx, contracts.Message{
    Role:    contracts.MessageRoleUser,
...
- `WithLogger(logger telemetry.Logger)` - Set custom logger
- `WithRetry(opts ...llm.RetryOption)` - Configure retry policy
- `WithBaseURL(url string)` - Set custom base URL
- `WithHTTPClient(client *http.Client)` - Use custom HTTP client

### Generation Options

- `contracts.WithTemperature(temp float64)` - Control randomness (0.0-1.0)
- `contracts.WithTopP(topP float64)` - Nucleus sampling parameter
...
- `WithMemory(memory contracts.Memory)` - Enable conversation memory
