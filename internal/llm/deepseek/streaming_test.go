...
func TestDeepSeekGenerateStream(t *testing.T) {
	...
	if events[0].Type != contracts.StreamEventMessageStart {
	...
	if lastEvent.Type != contracts.StreamEventMessageStop {
...
func TestDeepSeekGenerateStreamWithTools(t *testing.T) {
	// Create mock server
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		w.WriteHeader(http.StatusOK)
	...
	ctx := context.Background()
	eventChan, err := client.GenerateWithToolsStream(ctx, "Test", []contracts.Tool{testTool})
	if err != nil {
	...
	for _, event := range events {
		if event.Type == contracts.StreamEventToolUse {
			foundToolUse = true
		...
		if event.Type == contracts.StreamEventToolResult {
...
func TestDeepSeekGenerateStreamError(t *testing.T) {
	...
	foundError := false
	for _, event := range events {
		if event.Type == contracts.StreamEventError {
			foundError = true
			if event.Error == nil {
...
// deepSeekMockTool implements contracts.Tool for testing
type deepSeekMockTool struct {
	name        string
	description string
	result      string
	executeErr  error
...
func (m *deepSeekMockTool) Name() string {
	return m.name
}

func (m *deepSeekMockTool) Description() string {
	return m.description
}

func (m *deepSeekMockTool) Parameters() map[string]contracts.ParameterSpec {
...
func (m *deepSeekMockTool) Run(ctx context.Context, input string) (string, error) {
	if m.executeErr != nil {
		return "", m.executeErr
	}
	return m.result, nil
...
func (m *deepSeekMockTool) Execute(ctx context.Context, args string) (string, error) {
	if m.executeErr != nil {
		return "", m.executeErr
	}
	return m.result, nil
