...
// deepSeekMessageHistoryBuilder builds message history from memory
type deepSeekMessageHistoryBuilder struct {
	logger telemetry.Logger
}

// deepSeekNewMessageHistoryBuilder creates a new message history builder
func deepSeekNewMessageHistoryBuilder(logger telemetry.Logger) *deepSeekMessageHistoryBuilder {
	return &deepSeekMessageHistoryBuilder{
		logger: logger,
	}
}

// buildMessages builds messages from memory and current prompt
func (b *deepSeekMessageHistoryBuilder) buildMessages(ctx context.Context, prompt string, memory contracts.Memory) []Message {
	messages := []Message{}
...
func (b *deepSeekMessageHistoryBuilder) convertMemoryMessage(msg contracts.Message) Message {
	message := Message{
	...
	if msg.Role == contracts.RoleAssistant && len(msg.ToolCalls) > 0 {
	...
	if msg.Role == contracts.MessageRoleTool {
