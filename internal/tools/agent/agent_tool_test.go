...
type MockSubAgent struct {
	...
	runStreamFunc func(ctx context.Context, input string) (<-chan contracts.AgentStreamEvent, error)
}
...
func (m *MockSubAgent) RunStream(ctx context.Context, input string) (<-chan contracts.AgentStreamEvent, error) {
...
func (m *MockSubAgent) RunDetailed(ctx context.Context, input string) (*contracts.AgentResponse, error) {
	result, err := m.Run(ctx, input)
	if err != nil {
		return nil, err
	}
	return &contracts.AgentResponse{
		...
		ExecutionSummary: contracts.ExecutionSummary{
			LLMCalls:        1,
			ToolCalls:       0,
			SubAgentCalls:   0,
			ExecutionTimeMs: 100,
