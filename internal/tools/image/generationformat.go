...
func (t *GenerationTool) formatMultiTurnResponse(ctx context.Context, resp *contracts.ImageEditResponse, prompt string, isInitial bool) (string, error) {
	var result string
...
func (t *GenerationTool) formatImageURL(url string, image *contracts.GeneratedImage, index int) string {
	result := ""
	if index > 0 {
		result = fmt.Sprintf("\n--- Image %d ---\n", index+1)
	}
...
func (t *GenerationTool) formatImageBase64(image *contracts.GeneratedImage, index int) string {
	result := ""
	if index > 0 {
		result = fmt.Sprintf("\n--- Image %d ---\n", index+1)
	}
...
func (t *GenerationTool) formatResult(response *contracts.ImageGenerationResponse, prompt, imageURL string) string {
...
func (t *GenerationTool) formatResultWithBase64(response *contracts.ImageGenerationResponse, prompt string) string {
	result := fmt.Sprintf("Successfully generated image for prompt: \"%s\"\n\n", truncateString(prompt, 100))
