...
type ExtractTool struct {
	store contracts.GraphRAGStore
	llm   contracts.LLM
}

// NewExtractTool creates a graph extraction tool.
func NewExtractTool(store contracts.GraphRAGStore, llm contracts.LLM) *ExtractTool {
	return &ExtractTool{
		store: store,
		llm:   llm,
	}
...
func (t *ExtractTool) Parameters() map[string]contracts.ParameterSpec {
...
func (t *ExtractTool) Execute(ctx context.Context, args string) (string, error) {
	...
	opts := []contracts.ExtractionOption{
		graph.WithSchemaGuided(params.SchemaGuided),
	}
	if len(params.EntityTypes) > 0 {
		opts = append(opts, graph.WithExtractionEntityTypes(params.EntityTypes...))
	}
	if len(params.RelationshipTypes) > 0 {
		opts = append(opts, graph.WithExtractionRelationshipTypes(params.RelationshipTypes...))
	}
...
func formatExtractionResult(result *contracts.ExtractionResult, stored bool) string {
