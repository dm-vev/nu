...
// NewTool creates a graph search tool.
func NewTool(store contracts.GraphRAGStore) *Tool {
...
func (t *Tool) Execute(ctx context.Context, args string) (string, error) {
	...
	opts := []contracts.GraphSearchOption{
		graph.WithIncludeRelationships(true),
	}
	if len(params.EntityTypes) > 0 {
		opts = append(opts, graph.WithEntityTypes(params.EntityTypes...))
	}
	...
	case "local":
		...
		opts = append(opts, graph.WithMaxDepth(params.Depth))
	...
	default: // hybrid
		...
		opts = append(opts, graph.WithSearchMode(graph.SearchModeHybrid))
...
func formatSearchResults(results []contracts.GraphSearchResult) string {
