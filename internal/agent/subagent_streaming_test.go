...
func (m *StreamingMockLLM) Generate(ctx context.Context, prompt string, options ...contracts.GenerateOption) (string, error) {
	return m.responseContent, nil
}

func (m *StreamingMockLLM) GenerateWithTools(ctx context.Context, prompt string, tools []contracts.Tool, options ...contracts.GenerateOption) (string, error) {
	return m.responseContent, nil
}

func (m *StreamingMockLLM) GenerateDetailed(ctx context.Context, prompt string, options ...contracts.GenerateOption) (*contracts.LLMResponse, error) {
...
func (m *StreamingMockLLM) GenerateWithToolsDetailed(ctx context.Context, prompt string, tools []contracts.Tool, options ...contracts.GenerateOption) (*contracts.LLMResponse, error) {
...
func (m *StreamingMockLLM) GenerateStream(ctx context.Context, prompt string, options ...contracts.GenerateOption) (<-chan contracts.StreamEvent, error) {
...
func (m *StreamingMockLLM) GenerateWithToolsStream(ctx context.Context, prompt string, tools []contracts.Tool, options ...contracts.GenerateOption) (<-chan contracts.StreamEvent, error) {
...
func TestSubAgentStreaming(t *testing.T) {
	...
	t.Run("MainAgentStreaming", func(t *testing.T) {
		...
		var receivedEvents []contracts.AgentStreamEvent
		...
		if lastEvent.Type != contracts.AgentEventComplete {
	...
	t.Run("SubAgentToolStreaming", func(t *testing.T) {
		...
		var receivedEvents []contracts.AgentStreamEvent
		forwarder := func(event contracts.AgentStreamEvent) {
			receivedEvents = append(receivedEvents, event)
		}

		ctx := context.WithValue(context.Background(), contracts.StreamForwarderKey, contracts.StreamForwarder(forwarder))
...
func TestStreamForwarderContextKey(t *testing.T) {
	...
	retrievedForwarder, ok := ctx.Value(contracts.StreamForwarderKey).(contracts.StreamForwarder)
	if !ok {
		t.Fatal("Failed to retrieve stream forwarder from context")
	}

	// Test the forwarder works
	retrievedForwarder(contracts.AgentStreamEvent{
