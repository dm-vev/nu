...
const (
	...
	ExecutionPlanStatusPendingApproval ExecutionPlanStatus = "pending_approval"
...
func NewExecutionPlan(description string, steps []ExecutionPlanStep) *ExecutionPlan {
	now := time.Now()
...
// ExecutionPlanGenerator generates execution plans.
...
func ParseExecutionPlanFromResponse(response string) (*ExecutionPlan, error) {
	...
	steps := make([]ExecutionPlanStep, len(planData.Steps))
	for i, step := range planData.Steps {
		steps[i] = ExecutionPlanStep{
...
func CreateExecutionPlanPrompt(input string, tools []contracts.Tool) string {
