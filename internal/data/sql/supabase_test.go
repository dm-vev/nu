...
func setupSupabaseTestClient(t *testing.T) *datasql.SupabaseClient {
	...
	// Create client
	client, err := datasql.NewSupabase(url, apiKey, datasql.WithSupabaseDB(db))
	if err != nil {
...
func TestSupabaseInsertAndGet(t *testing.T) {
	client := setupSupabaseTestClient(t)
	defer func() {
...
func TestSupabaseUpdate(t *testing.T) {
	client := setupSupabaseTestClient(t)
	defer func() {
		if err := client.Close(); err != nil {
			t.Errorf("Failed to close client: %v", err)
...
func TestSupabaseQuery(t *testing.T) {
	client := setupSupabaseTestClient(t)
	defer func() {
		if err := client.Close(); err != nil {
			t.Errorf("Failed to close client: %v", err)
	...
	results, err := collection.Query(ctx, map[string]interface{}{
		"category": "test",
	}, contracts.QueryWithOrderBy("value", "asc"))
	if err != nil {
...
func TestSupabaseTransaction(t *testing.T) {
	client := setupSupabaseTestClient(t)
	defer func() {
		if err := client.Close(); err != nil {
			t.Errorf("Failed to close client: %v", err)
	...
	// Execute transaction
	err := client.Transaction(ctx, func(tx contracts.Transaction) error {
