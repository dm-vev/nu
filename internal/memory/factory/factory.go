...
// Factory provides factory functions to create memory instances from configuration
type Factory struct{}
...
func (f *Factory) CreateMemory(config map[string]interface{}, llmClient contracts.LLM) (contracts.Memory, error) {
	if config == nil {
...
func (f *Factory) createBufferMemory(config map[string]interface{}, llmClient contracts.LLM) (contracts.Memory, error) {
	...
	bufferOptions := []conversation.Option{conversation.WithMaxSize(bufferSize)}
	bufferMemory := conversation.NewConversationBuffer(bufferOptions...)
	...
	if llmClient != nil {
		if summaryEnabled, ok := config["enable_summarization"].(bool); ok && summaryEnabled {
			...
			summaryOptions := []conversation.SummaryOption{
				conversation.WithMaxBufferSize(maxBufferSize),
				conversation.WithSummaryLength(summaryLength),
			}

			// Wrap with summary memory if enabled
			return conversation.NewConversationSummary(llmClient, summaryOptions...), nil
		}
	}
...
func (f *Factory) createVectorMemory(config map[string]interface{}, llmClient contracts.LLM) (contracts.Memory, error) {
...
// NewFromConfig is a convenience function to create memory from config map
func NewFromConfig(config map[string]interface{}, llmClient contracts.LLM) (contracts.Memory, error) {
	factory := NewFactory()
