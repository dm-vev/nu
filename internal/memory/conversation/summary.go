...
type ConversationSummary struct {
	...
	maxBufferSize   int
	summaryMessages map[string]contracts.Message
...
func NewConversationSummary(llmClient contracts.LLM, options ...SummaryOption) *ConversationSummary {
	summary := &ConversationSummary{
		buffer:          NewConversationBuffer(),
		llmClient:       llmClient,
		maxBufferSize:   10, // Default max buffer size
		summaryMessages: make(map[string]contracts.Message),
...
func (c *ConversationSummary) AddMessage(ctx context.Context, message contracts.Message) error {
	c.mu.Lock()
	defer c.mu.Unlock()
	...
	// Get conversation ID
	conversationID, err := ConversationKey(ctx)
	if err != nil {
		return err
	}
	...
	if len(messages) >= c.maxBufferSize {
		...
		// Store summary
		c.summaryMessages[conversationID] = contracts.Message{
			Role:    "system",
			Content: summary,
...
func (c *ConversationSummary) GetMessages(ctx context.Context, options ...contracts.GetMessagesOption) ([]contracts.Message, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	...
	result := []contracts.Message{summary}
...
func (c *ConversationSummary) summarize(ctx context.Context, messages []contracts.Message) (string, error) {
	...
	summary, err := c.llmClient.Generate(ctx, sb.String(), func(o *contracts.GenerateOptions) {
