...
func (m *Menu) renderItems(width int) []string {
	if len(m.filtered) == 0 {
	...
	if selected, ok := m.Selected(); ok {
		...
		lines = append(lines, m.line(width, m.opts.Muted("  Model Name: ")+m.opts.Text(modelDisplayName(selected))))
	}
	return lines
}

func (m *Menu) renderItem(entry model.Model, selected bool) string {
	prefix := "  "
	id := entry.ID
	if selected {
		prefix = "> "
...
func (m *Menu) border(width int) string {
	return m.opts.Border(strings.Repeat("─", width))
}

func (m *Menu) line(width int, text string) string {
