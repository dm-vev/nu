...
func TestToolBlockRendersBashCommandAndOutput(t *testing.T) {
	block := New(
		"bash",
		"call-1",
		`{"command":"pwd"}`,
...
func TestToolBlockRendersFailedCommandWithErrorBackground(t *testing.T) {
	block := New(
		"bash",
		"call-1",
		`{"command":"false"}`,
...
func TestToolBlockRendersPatchDiffColors(t *testing.T) {
	block := New(
		"edit",
		"call-1",
		`{"path":"a.go"}`,
