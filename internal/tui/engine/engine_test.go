...
func TestEngineEngineRendersComponentTreeAndDiffs(t *testing.T) {
	var out bytes.Buffer
	term := terminal.New(nil, &out, 20, 10)
	ui := New(term, Options{Title: "Nu"})
	label := components.NewText("hello", components.TextOptions{})
	ui.AddChild(label)
...
func TestEngineEngineRendersBottomViewportWhenContentOverflows(t *testing.T) {
	var out bytes.Buffer
	term := terminal.New(nil, &out, 20, 3)
	ui := New(term, Options{Title: "Nu"})
	ui.AddChild(components.NewText("top", components.TextOptions{}))
	ui.AddChild(components.NewText("middle", components.TextOptions{}))
	ui.AddChild(components.NewText("bottom", components.TextOptions{}))
	ui.AddChild(components.NewText("footer", components.TextOptions{}))
...
func TestEngineEngineScrollsOverflowingViewport(t *testing.T) {
	var out bytes.Buffer
	term := terminal.New(nil, &out, 20, 3)
	ui := New(term, Options{Title: "Nu"})
	ui.AddChild(components.NewText("top", components.TextOptions{}))
...
func TestEngineEngineDiffUsesAbsoluteRowsNearBottom(t *testing.T) {
	var out bytes.Buffer
	term := terminal.New(nil, &out, 20, 3)
	ui := New(term, Options{Title: "Nu"})
	label := components.NewText("a", components.TextOptions{})
	ui.AddChild(components.NewText("top", components.TextOptions{}))
	ui.AddChild(components.NewText("middle", components.TextOptions{}))
	ui.AddChild(label)
