...
|---|---|---|
| Full imported SDK baseline | IMPLEMENTED | complete upstream `pkg/` feature source and owning tests currently under `internal/` |
...
| SDK dependency-direction check | IMPLEMENTED | structural tests cover approved child packages and keep `internal/agent` independent of concrete transports |
| Generated protobuf relocation/regeneration | IMPLEMENTED | schema and generated output live in `internal/transport/grpc/pb` |
| SDK agent runtime | IMPLEMENTED | `internal/agent`, upstream tests |
...
| Old Nu agent loop | REMOVED | `internal/agent` is upstream package |
