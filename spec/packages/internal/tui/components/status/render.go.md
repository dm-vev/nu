# `internal/tui/components/status/render.go`
...
- `TestStatusAlwaysReservesOneLine` fails if idle status stops reserving the row.
- `TestStatusCanUseASCIIFrames` fails if configured ASCII frames are ignored.

### `func alertStyle(value string, frame int) string`
